  ...
  const plans = [
    {
      ...
      features: ["Jusqu'à 100 utilisateurs", "TOTP illimité", "Support communauté"],
      featured: false,
    },
    {
      name: "Business",
      ...
      <section className="container mx-auto px-6 py-20">
        <div className="text-center max-w-2xl mx-auto">
          ...
          <p className="mt-5 text-lg text-muted-foreground">
            Commencez gratuitement, payez à l'usage quand vous grandissez.
          </p>
        </div>
        <div className="mt-16 grid md:grid-cols-3 gap-6 max-w-5xl mx-auto">
          {plans.map((p) => (
            <div
              key={p.name}
              className={`relative rounded-2xl border p-8 ${p.featured ? "border-primary bg-card shadow-[var(--shadow-elegant)]" : "bg-card shadow-[var(--shadow-card)]"}`}
            >
              {p.featured && (
              ...
              <Button
                asChild
                className={`mt-6 w-full ${p.featured ? "bg-[image:var(--gradient-primary)]" : ""}`}
